#!/usr/bin/env python3
"""Verify adventure-verifier campaign artifacts and re-run every stored case."""
...
from adventure_verifiers.campaign import (
    CASE_SCHEMA,
    RECEIPT_SCHEMA,
    SUMMARY_SCHEMA,
    run_cases,
...
def main() -> None:
    ...
    source_manifest = load_json(root / "frozen_source_manifest.json")
    ...
    if taxonomy.get("schema") != "alife.adventure.verifier_taxonomy.v1":
    ...
    for row in rows:
        ...
        if canonical_sha256(suite) != canonical_sha256(row.get("verification")):
            suite_mismatches += 1
        matched = bool(suite["accepted"]) == bool(row["expected_accept"])
        if not matched:
    ...
    if not args.skip_source_replay:
        replay_manifest = copy.deepcopy(manifest)
        replay_manifest["design"]["source_manifest"] = "frozen_source_manifest.json"
        replay_rows, replay_results = run_cases(replay_manifest, root / "frozen_manifest.json")
        ...
        if not passed:
            errors.append("source episode and case replay did not byte-project exactly")
    ...
    if sha256_file(root / "frozen_source_manifest.json") != receipt.get(
        "source_manifest_sha256"
    ):
        errors.append("source manifest hash does not match receipt")
