...
**Proposition 1 — preservation of reachable possibilities.** Suppose \(F^\#\) is sound, so the post-step reachable set satisfies \(R_{t+1}\subseteq\gamma(a^-_{t+1})\). Suppose a verifier also establishes \(R_{t+1}\subseteq\gamma(\widehat a_{t+1})\), and the domain satisfies \(\gamma(a\wedge b)=\gamma(a)\cap\gamma(b)\). Then
...
The proof is the set inclusion defining the meet. The useful content is operational: all assumptions must be recorded, especially which time-indexed reachable set the old lattice describes.
...
\[
A_{\mathcal C}(b)=igcap_{j\in J_H}\{u:g_j(b,u)\le0\}.
\]
...
\[
(\mathcal C_k,\Omega_t)
\xrightarrow[ho_E,\rho_N,\rho_R]{\operatorname{Amend}}
\mathcal C_{k+1}.
\]
